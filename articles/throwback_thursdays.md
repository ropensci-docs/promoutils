...
Throwback Thursday (TT) posts are sent via our social media accounts and
highlight a historical blog post.

We use the matomo package to review Matomo blog view metrics and choose
a post that which fits any or some of the following:

- Was published in the same month as the TT post
- Was published a milestone of years ago (5, 10, 15, etc.)
- Was an important announcement (first Champions Program, first Dev
  guide published, etc.)
- Something specifically related to rOpenSci as an organization
- Was a popular post
- Is a perennial theme/idea (i.e. Software Peer Review)

Pick posts that have not been (or have not recently been) highlighted in
a TT.

We track TT posts in a [private project on
`rosadmin`](https://github.com/orgs/rosadmin/projects/9/views/1) so we
can check if we’ve already used a post in the past.
...
No we can review the ‘popularity’ of historical posts for a particular
month (here, we review for May, the 5th month). In Positron, if the
window is wide enough to show the full URL, you should be able to click
on the links to open the blog post in your browser to help you decide.
...
Once we’ve decided on a post, we can use the
[`tt_post()`](https://docs.ropensci.org/promoutils/reference/tt_post.md)
function to grab the relevant details and prepare the code for a series
of `socials_post_issues()`
...
We paste this code into an R script. Note that while it automatically
gets the post information and the date when this TT should be posted, we
still need to flesh this post out and add some details.
...
Now we can run this code which will create the draft post issues in the
GitHub repository. It will also open these issues in the browser, don’t
forget to review and remove the ‘draft’ and ‘needs-review’ labels to
